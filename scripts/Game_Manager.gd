...
var _ai_dog:GuideDog
var _ai_dog_owner:DogDowner

var initialized:bool = false
var dog_position_offset:Vector2
var ai_dog_position_offset:Vector2
var _ui:UIManager
var objectives = {}
...
func _process(delta: float) -> void:
	...
	print("npc spawn: ", npc_spawn.name)
	if (npc_spawn.global_position - _dog.global_position).length() < 100 \
		and _ai_dog.move_direction == Vector2.ZERO:
			trigger_ai_dog()
...
func recalculate_ai_dog_position_offset():
	ai_dog_position_offset = _ai_dog_owner.global_position - _ai_dog.global_position

func trigger_ai_dog():
	_ai_dog.can_move = true
	
func initialize_game():
	recalculate_dog_position_offset()
	recalculate_ai_dog_position_offset()
	_dog_owner.initialize(_dog)
	_ai_dog_owner.initialize(_ai_dog)
	_dog.initialize(_dog_owner)
	_ai_dog.initialize(_ai_dog_owner)
	_ui.start_game()
	initialized = true
	objectives["find_food"] = false
	objectives["find_bench"] = false
	npc_spawn = get_tree().current_scene.get_node("Npc_Spawn")
	saveCheckPoints()
	

func place_dog_food():
	var posx = randi_range(13,375)
	var posy = randi_range(1000,1175)
	...
	while (!result.is_empty() and  can_place_dog_food(v,result)) or (v-_dog.global_position).length() < 200:
		posx = randi_range(13,375)
		posy = randi_range(1000,1175)
		v = Vector2(posx,posy)
